...
export default function ToolPage({
  ...
  const handleEditConfig = async () => {
    try {
      const files = await window.electronAPI.config.readConfigFiles(toolType)
  ...
  const handleSaveConfig = async (files: ConfigFile[]) => {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              管理 {toolName} 的 API 配置，当前共 {providers.length} 个服务商
          ...
          <div className="flex gap-2">
            ...
            <button onClick={onAdd} className={getBrandButtonWithIcon(toolType)}>
        ...
        <div className="mt-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="搜索服务商..."
              className={`w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg ${getBrandInputFocus(toolType)}`}
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {providers.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-500">
            ...
            <p className="text-lg font-medium mb-2">还没有 {toolName} 服务商</p>
