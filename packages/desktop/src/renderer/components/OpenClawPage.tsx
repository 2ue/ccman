...
export default function OpenClawPage({
  ...
  const handleEditConfig = async () => {
    try {
      const files = await window.electronAPI.config.readConfigFiles('openclaw')
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-white">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">OpenClaw 服务商管理</h1>
            <p className="text-sm text-gray-500 mt-1">
              管理 OpenClaw 的 API 配置，当前共 {providers.length} 个服务商
          ...
          <div className="flex gap-2">
            ...
            <button onClick={onAdd} className={getBrandButtonWithIcon('openclaw')}>
        ...
        <div className="mt-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="搜索服务商..."
              className={`w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg ${getBrandInputFocus('openclaw')}`}
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {providers.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-500">
            ...
            <p className="text-lg font-medium mb-2">还没有 OpenClaw 服务商</p>
