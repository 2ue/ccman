...
describe('OpenClaw ToolManager', () => {
  ...
  it('should manage openclaw providers and write target configs on switch', () => {
    ...
    expect(openclawConfig.agents.defaults.model.primary).toBe(`${providerName}/gpt-5.5`)
    expect(openclawConfig.agents.defaults.imageModel).toBe(`${providerName}/gpt-5.5`)
    ...
    expect(modelsConfig.providers[providerName].models).toHaveLength(1)
  })
})
