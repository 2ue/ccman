...
| `network_access` | 联网能力（按 Codex 新版配置） | ✅ 默认 `enabled`（用户可覆盖） |
...
// 6. 更新 auth.json（先备份，再覆盖写入，仅保留 OPENAI_API_KEY）
if (fileExists(authPath)) {
  fs.copyFileSync(authPath, `${authPath}.bak`)
}
fs.writeFileSync(authPath, JSON.stringify({ OPENAI_API_KEY: provider.apiKey }, null, 2))
```
