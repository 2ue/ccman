...
```json
{
  "env": {
    ...
    "ANTHROPIC_BASE_URL": "https://gmn.chuangzuoli.cn/openai",
...
| 字段 | 作用 | ccman 管理 |
|------|------|-----------|
| `model_provider` | 当前使用的服务商名称 | ✅ 设置为 provider.name |
| `model` | 当前使用的模型 | ✅ 设置为 provider.model 或保留现有值 |
| `model_providers[name]` | 服务商配置 | ✅ 添加/更新当前 provider |
| `model_reasoning_effort` | 推理强度 | ❌ 保留用户配置 |
| `disable_response_storage` | 禁用响应存储 | ❌ 保留用户配置 |
| `sandbox_mode` | 沙箱模式 | ❌ 保留用户配置 |
| `approval_policy` | 审批策略 | ❌ 保留用户配置 |
| `features` | 功能开关 | ❌ 保留用户配置 |
