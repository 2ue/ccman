...
```json
{
  ...
  "env": {
    ...
    "ANTHROPIC_BASE_URL": "https://ai.gmncode.com",
...
| 字段                                           | 作用                                               | ccman 管理         |
| ---------------------------------------------- | -------------------------------------------------- | ------------------ |
| `model`                                        | Claude Code 默认模型，`sonnet` 跟随官方最新 Sonnet | ✅ 默认值 "sonnet" |
...
- ✅ 用户的其他 `env` 变量不会丢失
- ✅ 用户手动设置的 `model` 不会丢失；新配置默认使用 `sonnet`
- ✅ 认证字段始终是最新的
...
2. 从顶层 `model` 获取默认模型
3. 从 `env.ANTHROPIC_AUTH_TOKEN` 获取 API Key
4. 从 `env.ANTHROPIC_BASE_URL` 获取 Base URL
5. 使用这些配置连接 API 服务
...
| `GEMINI_MODEL`               | 模型名称      | ✅ 新配置默认 `gemini-3.5-flash`；`provider.model` 可覆盖 |
...
if (provider.model) {
  try {
    ...
    // provider 元数据优先于模板默认值；已有用户自定义模型保留
    if (meta.defaultModel && !existingGeminiModel) {
...
- ✅ 用户的其他 `.env` 变量不会丢失
- ✅ 非覆盖模式会保留已有 `GEMINI_MODEL`，新配置默认写入 `gemini-3.5-flash`
- ✅ 支持 JSON 元数据（高级用法）
...
| `provider[key].models`          | 模型配置        | ✅ 默认包含 gpt-5.5 变体   |
...
| 工具        | 配置文件                                       | 认证字段                                                          | 其他重要字段                                                    |
| ----------- | ---------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------- |
| Claude Code | `~/.claude/settings.json`                      | `env.ANTHROPIC_AUTH_TOKEN`<br>`env.ANTHROPIC_BASE_URL`            | `model`<br>`permissions`                                        |
...
| Gemini CLI  | `~/.gemini/settings.json`<br>`~/.gemini/.env`  | `GEMINI_API_KEY`<br>`GOOGLE_GEMINI_BASE_URL`                      | `GEMINI_MODEL`<br>`ide.enabled`<br>`security.auth.selectedType` |
